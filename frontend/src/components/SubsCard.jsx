...
const SubsCard = ({price}) => {
  ...
  const defaultPClass = "text-md flex flex-row items-center gap-2"
  return (
    <div className={`flex flex-col w-[350px] h-[400px] rounded-lg outline-1 outline-neutral-300 items-center transition-all transform duration-600 ease-in-out hover:translate-y-[-15px] ${currentTheme === "dark" ? "bg-neutral-900 text-white" : "bg-white"}`}>
        <div className={`flex bg-blue-400 w-[350px] h-[100px] items-center justify-center transition-all ease-in-out duration-300 ${currentTheme === "dark" ? "hover:text-black hover:bg-white" : "hover:text-white hover:bg-neutral-700"} rounded-t-lg`}>
        ...
        <button className={`bg-blue-400 rounded-md px-[12px] py-[2px] mt-[15%] cursor-pointer duration-400 ${currentTheme === "dark" ? "hover:bg-white hover:text-black" : "hover:bg-neutral-700 text-white"}`} onClick={paymentIntent(price)}>Subscribe</button>
    </div>
  )
}
