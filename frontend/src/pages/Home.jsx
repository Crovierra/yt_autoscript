...
const Home = () => {
  ...
  return (
    <div className={`flex flex-col justify-center items-center min-h-screen overflow-y-auto pt-[100px] max-sm:pt-[90px] max-sm:scale-0.7 ${currentTheme === "dark" ? "bg-neutral-700 text-white" : ""} duration-300`}>
      ...
      <p className="w-[800px] max-w-screen h-[auto] text-center my-[1%] max-sm:w-[400px] max-xs:w-[300px] max-md:w-[600px] max-sm:py-[2%] px-[2%]">Transform your YouTube videos into text transcripts instantly. Professional-grade conversion that's free, lightning-fast, and incredibly simple to use.</p>
      <div className="flex flex-row justify-center items-center gap-6 my-[2%] text-gray-600">
        ...
        <div className={`flex flex-col max-sm:flex-col rounded-2xl w-[800px] py-[2%] max-sm:my-[20px] max-sm:py-[4%] h-auto max-md:w-[600px] gap-2 max-sm:w-[300px] max-lg:w-[700px] max-xs:bg-red-900 justify-center items-center shadow-md ${currentTheme === "dark" ? "bg-neutral-900" : ""}`}>
          <div className="flex flex-row gap-4 max-sm:flex-col items-center max-sm:pt-[2%]">
            <input placeholder="Paste Youtube Video URL here" className={`bg-gray-100 w-[400px] max-sm:max-w-[250px] outline px-[10px] h-[40px] rounded-md outline-gray-300 active:outline-blue-400 ${currentTheme === "dark" ? "bg-white text-black" : ""}`} onChange={handleLink}></input>
            <button 
